...
const authController = async (req, res) => {
  try {
    ...
    if (
      !detailsfromtoken.id ||
      !detailsfromtoken.email ||
      !detailsfromtoken.name
    ) {
      return res.status(400).json({
    ...
    return res.redirect(`http://localhost:3000/loginpage?token=${jwtToken}`);
  } catch (error) {
    return res.redirect(`http://localhost:3000/loginpage?error=auth_failed`);
  }
};
